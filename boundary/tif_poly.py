...
def raster2poly(img_folder):
    files = os.listdir(img_folder)
    save_folder = img_folder + '_poly'
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    for file in files:
        if file[-4:] == ".tif":
            raster = os.path.join(img_folder, file)
            ...
            outshp = os.path.join(save_folder, file.replace('.tif', '.shp'))
...
img_folder = r'D:\data\sample_farm2\sg\kuang_grey_proj'
raster2poly(img_folder)